...
// Extract a bracketed block: [ commands ]
// Returns pointer to character after closing bracket ']'
const char* extract_block(const char *str, char *buffer, size_t buf_size) {
    str = skip_whitespace(str);
    if (*str != '[') return NULL; // Expected block start
...
void eval_logo(LogoApp *app, const char *code) {
    ...
    while (*ptr != '\0') {
        ...
        if (strcasecmp(token, "REPEAT") == 0) {
            ...
            if (sscanf(ptr, "%d%n", &count, &read_bytes) == 1) {
                ...
                if (ptr != NULL) {
                    // Recursively execute the block body N times
    ...
    // Refresh GTK Canvas once the line or loop completes
